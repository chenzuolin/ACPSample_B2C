...
int aa = (Integer)request.getSession().getAttribute("id");
		CommonlyDao dao8 = new CommonlyDao();
		List<Commonly> list8 = dao8.findUserByID(aa);
		for(Commonly commonly : list8){
			int aaa = commonly.getGreens_ID();
			int bbb = commonly.getCommonly_ID();
			GreensDao dao = new GreensDao();
			List<Greens> list3 = dao.findUserByID(aaa);
	...
	%>
      <tr>
      <td style="text-align:center;display:none;"><%=aaa %></td>
  ...
  <td style="text-align:center;padding:4px 0px;line-height:26px;width:130px;">
	  ...
	  <a href="RR1?id=<%=bbb %>"style="" title="删除"><i class="layui-icon">&#xe640;</i></a>
  </td>
</tr>
