...
<div style="margin-left:135px;margin-top:20px;">
	<c:choose>
		...
		<c:otherwise>
			<a href="UserController5?currentPage=${page.prevPage}" id="prevPage">上一页</a>
		</c:otherwise>
		</c:choose>
        <c:forEach items="${page.showPageNums}" var="pageNum">
            <a href="UserController5?currentPage=${pageNum}" id="pageNum">${pageNum}</a>
        </c:forEach>
        <a href="UserController5?currentPage=${page.nextPage}" id="nextPage">下一页</a>
	</div>
</body>
</html>