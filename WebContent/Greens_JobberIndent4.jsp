...
<table  class="layui-table"  style=" width:80%; margin-left:10%; table-layout: fixed;">
	...
	<tr>
		...
		<td style="text-align:center;"><a href="GreensJobberIndexServlet?id=${indent.indent_ID }" class="layui-btn" style="padding: 0; margin: 0px ; width: 80px; font-size: 7px; height:25px; line-height: 25px ;">分拣完毕</a>
	</td>
	</tr>
	</c:forEach>
</table>
<div style="margin-left:135px;margin-top:20px;">
	<c:choose>
		...
		<c:otherwise>
			<a href="UserControllers7?currentPage=${page.prevPage}" id="prevPage">上一页</a>
		</c:otherwise>
		</c:choose>
        <c:forEach items="${page.showPageNums}" var="pageNum">
            <a href="UserControllers7?currentPage=${pageNum}" id="pageNum">${pageNum}</a>
        </c:forEach>
        <a href="UserControllers7?currentPage=${page.nextPage}" id="nextPage">下一页</a>
	</div>
</body>
</html>