...
<div class="layui-card">
  <div class="layui-card-header" style="background-color:#009688;color: white;font-size:15px;height: 30px ; line-height: 30px;">正在分拣订单
...
<div style="margin-left:135px;margin-top:20px;">
	<c:choose>
		...
		<c:otherwise>
			<a href="UserController3?currentPage=${page.prevPage}" id="prevPage">上一页</a>
		</c:otherwise>
		</c:choose>
        <c:forEach items="${page.showPageNums}" var="pageNum">
            <a href="UserController3?currentPage=${pageNum}" id="pageNum">${pageNum}</a>
        </c:forEach>
        <a href="UserController3?currentPage=${page.nextPage}" id="nextPage">下一页</a>
	</div>
</body>
</html>