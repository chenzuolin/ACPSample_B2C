...
<style>
	th{color: black;}
	td{color:blackl; text-align: center;}
</style>

</head>
<body>
		<div class="layui-card">
  <div class="layui-card-header" style="background-color:#009688;color: white;font-size:15px;height: 30px ; line-height: 30px;">待接收订单
  </div>
</div>
...
<tr>
		...
		<td style="text-align:center;"><a href="GreensJobberServlet?id=${indent.indent_ID }" class="layui-btn" style="padding: 0; margin: 0px ; width: 80px; font-size: 7px; height:25px; line-height: 25px ;" >接收订单</a>
	</td>
	</tr>
	</c:forEach>
</table>
<div style="margin-left:135px;margin-top:20px;">
	<c:choose>
		...
		<c:otherwise>
			<a href="UserControllers6?currentPage=${page.prevPage}" id="prevPage">上一页</a>
		</c:otherwise>
		</c:choose>
        <c:forEach items="${page.showPageNums}" var="pageNum">
            <a href="UserControllers6?currentPage=${pageNum}" id="pageNum">${pageNum}</a>
        </c:forEach>
        <a href="UserControllers6?currentPage=${page.nextPage}" id="nextPage">下一页</a>
	</div>
</body>
</html>