...
<form name="form1" action="UserControllerss2" class="layui-form layui-form-pane" method="get" style="margin-top: 15px;">
 <div class="nva">
...
<div style="margin-left:135px;margin-top:20px;">
	<c:choose>
		...
		<c:otherwise>
			...
			<a href="UserControllerss1?currentPage=${page.prevPage}&d=<%=hhh %>" id="prevPage">上一页</a>
		</c:otherwise>
		</c:choose>
        <c:forEach items="${page.showPageNums}" var="pageNum">
            <a href="UserControllerss1?currentPage=${pageNum}" id="pageNum">${pageNum}</a>
        </c:forEach>
        <a href="UserControllerss1?currentPage=${page.nextPage}&d=<%=hhh %>" id="nextPage">下一页</a>
	</div>
