...
/**
 * Servlet implementation class LL1Servlet
 */
@WebServlet("/LL1Servlet")
public class LL1Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Indent indent : list) {
			...
			for(Order order:list1){
				...
				for(Greens greens:list2){
					...
					float aa = greens.getGreens_Market_Price();
		...
		for(oo o1 : lists){
			...
			for(Greens green : list15){
				...
				float rr = total* greens_Numbe;
				zzzz zzz = new zzzz();
				zzz.setAa(greens_Name);
				zzz.setBb(greens_Numbe);
